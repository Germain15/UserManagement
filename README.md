...
- [User Management API](#user-management-api)
  ...
  - [Détail des jobs](#détail-des-jobs)
  - [Versionnement des images](#versionnement-des-images)
...
Lance un container PostgreSQL via services: GitHub Actions. Exécute mvn clean test avec les variables de connexion injectées en environnement. Si un test échoue, le pipeline s'arrête — aucun build ne se lance sur du code cassé.

- Job build-and-push
  
Conditionné au succès de tests (needs: tests). Build l'image Docker et la pousse vers Docker Hub avec 3 types de tags. Skippé sur les Pull Requests pour ne pas publier de code non encore mergé.

- Job deploy-staging
  
Déclenché uniquement sur main. Simule le déploiement avec les commandes SSH réelles documentées en commentaire — prêtes à être décommentées avec les secrets correspondants.

- Job deploy-production
  
Déclenché uniquement sur un tag v*.*.*. Jamais sur un push ordinaire — exige une décision consciente (git tag v1.0.0). Inclut une étape de vérification pré-déploiement et un guide de rollback.

DOCKER_USERNAME   votre username Docker Hub
DOCKER_PASSWORD   votre Access Token Docker Hub (pas le mot de passe principal)
